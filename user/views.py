...
def suscribed():
    ...
    if not is_already_suscribed:
        if user_email and suscribed_to:
            ...
            today = date.today()
            registers.insert_database(user_email=user_email, suscribed_to=suscribed_to, suscribed_from=suscribed_from, suscribed_date=today)
...
def register():
    if request.method == "POST":
        ...
        if not is_already_registered:
            ...
            users_db.insert_database(user_email=user_email, user_name=user_name, user_password=user_password, user_id=user_id)
